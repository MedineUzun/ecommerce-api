...
4. Terminalinizi (veya komut satırını) açın ve çıkarttığınız klasörün bulunduğu konuma gelerek projenin içine girmek için şu komutu çalıştırın.
```bash
...
- **Bulut Veritabanı (MongoDB Atlas) Kullanıyorsanız:**
  ...
  4. Alt kısımda size verilen bağlantı cümlesini kopyalayın. Şöyle görünmelidir: 
     `mongodb+srv://KULLANICI_ADINIZ:SIFRENIZ@cluster0.abcde.mongodb.net/DATABASE_NAME?retryWrites=true&w=majority`
  5. Kopyaladığınız bu metni `env` dosyasında `MONGODB_URI=` eşittir işaretinden hemen sonraya yapıştırın. *Linkte `<password>` yazan yeri silip kendi veritabanı şifrenizi eklemeyi unutmayın!*
     > **Önemli Not:** Bağlantı linkinin sonundaki `DATABASE_NAME` yazısını silerek yerine kurulacak olan **veritabanınızın ismini** yazmalısınız.
- **Kendi Bilgisayarınıza Kurulu MongoDB Kullanıyorsanız:**
  ...
  > **Önemli Not:** Buradaki `DATABASE_NAME` kısmını bağlanılacak **veritabanının adıyla** değiştirmelisiniz.
...
Örnek: `JWT_SECRET=super_gizli_projem_icin_karma_isaret123!`
...
```bash
# Geliştirici modu(Kodda bir şeyi değiştirip kaydettiğinizde proje otomatik yeniden başlar)
npm run start:dev
